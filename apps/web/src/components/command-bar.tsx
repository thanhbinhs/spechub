...
const modes = [
  {
    ...
    label: "Search",
    icon: Search,
    placeholder: "Search model, chipset, battery...",
  },
  {
    value: "ai" as const,
    label: "Ask AI",
    icon: BrainCircuit,
    placeholder: "Ask a catalog question...",
  },
];
      ...
      <div className="flex flex-col gap-2 md:flex-row md:items-center">
        ...
        <div className="relative min-w-0 flex-1">
          ...
          <input
            ...
            className={clsx(
              "w-full rounded-md border border-transparent bg-slate-50 pl-10 pr-3 text-sm outline-none transition focus:border-blue-500 focus:bg-white",
              compact ? "h-9" : "h-11",
            )}
          />
        </div>
        ...
        <button
          className={clsx(
            "inline-flex items-center justify-center gap-2 rounded-md bg-blue-600 px-4 text-sm font-medium text-white transition hover:bg-blue-700",
            compact ? "h-9" : "h-11",
          )}
        >
          <SendHorizontal size={16} />
          <span>{mode === "ai" ? "Ask" : "Go"}</span>
        </button>
      </div>
    </form>
  );
