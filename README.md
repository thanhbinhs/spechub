...
The local answer engine recognizes comparison, ranking, recommendation, and focused lookup questions. It turns stated use cases (for example gaming, photography, travel, productivity, or long-term ownership) into relevant decision criteria and produces conditional trade-offs instead of returning a raw specification dump. Broad comparisons retain one complete, explicitly selected variant per device and cover every populated core category; focused follow-ups keep only the requested criteria. A hosted answer provider uses the same verified analytical draft and decision brief, and an answer that omits required comparison categories or fails grounding falls back to that deterministic draft.
